...
from tasks.t2t_translate import TransformerTranslator
import socket
import atexit
import signal
...
class MTMonkeyTransformerWorker(object):
    ...
    def __init__(self, config, logger):
        ...
        # Transformer translator (queries a Transformer model server)
        # translate_host, translate_port, source_lang, target_lang, model_name, problem, t2t_usr_dir, data_dir, preprocess_cmd, postprocess_cmd
        self._translator = TransformerTranslator(config.get('TRANSLATE_HOST'),
                                            config.get('TRANSLATE_PORT'),
                                            config.get('SOURCE_LANG', 'en'),
                                            config.get('TARGET_LANG', 'en'),
                                            config.get('MODEL_NAME', ''),
                                            config.get('PROBLEM', ''),
                                            config.get('T2T_USR_DIR', ''),
                                            config.get('DATA_DIR', ''),
...
def main():
    ...
    # load configuration
    config = ConfigObj("t2t_worker.cfg")
    ...
    server.register_instance(MTMonkeyTransformerWorker(config, logger))
